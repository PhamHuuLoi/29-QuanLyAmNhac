...
public class SwipeMenuLayout extends FrameLayout {
    ...
    public boolean onSwipe(MotionEvent event) {
        ...
        } else if ((this.isFling || Math.abs(((float) this.mDownX) - event.getX()) > ((float) (this.mMenuView.getWidth() / 2))) && Math.signum(((float) this.mDownX) - event.getX()) == ((float) this.mSwipeDirection)) {
            smoothOpenMenu();
        } else {
            smoothCloseMenu();
            return false;
    ...
    private void swipe(int dis) {
        if (Math.signum((float) dis) != ((float) this.mSwipeDirection)) {
            dis = 0;
